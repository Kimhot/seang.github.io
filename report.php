                ...
                <table id="example11" class="table table-bordered table-hover">
                    ...
                    <tbody>
                      <?php
                        ...
                        // $dateLoop = $day."-".$month."-".$year;
                        $dateLoop = $year."-".$month."-".$day;
                        while($day <= 31){
                          $sellAmount = mysqli_fetch_assoc($conn->query("SELECT SUM(total_price) as pr FROM `p_invoice_summary` WHERE `user` = '$_SESSION[user_id]' and order_date = '$dateLoop' group by order_date"));
                          $totalqty = mysqli_fetch_assoc($conn->query("SELECT SUM(total_qty) as tq FROM `p_invoice_summary` WHERE `user` = '$_SESSION[user_id]' and order_date = '$dateLoop' group by order_date"));

                      ?>
                      <tr>
                        <td><?php echo $dateLoop; ?></td>
                        <td><?php echo isset($sellAmount['pr']) ? $sellAmount['pr'] : "0.00"; ?></td>
                        <td><?php echo isset($totalqty['tq']) ? $totalqty['tq'] : "0.00"; ?></td>
                    ...
                    <tfoot class="bg-light">
                      <tr>
                        ...
                        <td><strong id="totalqty">0.00</strong></td>
